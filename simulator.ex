defmodule RoverApp.Simulator do
  ...
  alias RoverApp.UserSup
  alias RoverApp.Rover
  alias RoverApp.RoverSup
  ...
  def handle_cast({:create_user, user_name}, state) do
    case UserSup.create_user(user_name) do
      {:ok, _pid} ->
        Logger.info("User created successfully!")
        {:noreply, %{state | user: state.user ++ user_name}}

      {:error, error} ->
        Logger.warn(error, label: "User couldn't created, error: ")
        {:noreply, state}
    end
  end
  ...
  def handle_cast({:init_rover, rover_id, {x, y}}, state) do
    case RoverSup.create_rover(rover_id) do
      {:ok, _pid} ->
      ...
      {:error, error} ->
        Logger.warn("Rover #{rover_id} couldn't initialize error: #{inspect(error)}")
        {:noreply, state}
    end
  end
  ...
  def handle_cast({:stop_rover, rover_id}, state) do
    case Rover.unreserve(rover_id) do
      ...
      {:error, error} ->
        Logger.warn("Rover #{rover_id} couldn't stopped error: #{inspect(error)}")
        {:noreply, state}
    end
  end
